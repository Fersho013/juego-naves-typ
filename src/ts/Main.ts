...
function update(){
    ...
    bosses.forEach((b,i)=>{
        ...
        if(b.type==='moving' || b.id==='B2'){
            ...
            if(!b.transformed && b.hp<=0){ b.transformed=true; b.immune=true; b.vx=0; b.vy=0; b.transformPhase='toCenter'; floatingTexts.push({x:b.x,y:b.y-70,text:'⚡ COLAPSO — FASE 2 ⚡',life:3.0,color:'#ffff00'}); fxState.screenShake=12; setTimeout(()=>{ const bi2=bosses.indexOf(b); if(bi2!==-1) bosses.splice(bi2,1); bosses.push({id:'DOPPEL_Y',type:'doppel_y' as const,x:canvas.width*0.3,y:140,targetY:140,hp:3000,maxHp:3000,vx:0,vy:0,lastMissile:Date.now(),lastSpiral:Date.now(),spiralActive:false,spiralCount:0,parryCooldown:false,parryActive:false,enraged:false,bombs:0} as unknown as Boss); bosses.push({id:'DOPPEL_O',type:'doppel_o' as const,x:canvas.width*0.7,y:140,targetY:140,hp:3000,maxHp:3000,vx:0,vy:0,dashState:'chase',dashTimer:0,dashVx:0,dashVy:0,dashCount:0,maxDashes:3,lastDashTime:Date.now(),lastExplosion:0,explodeCooldown:false,enraged:false,bombs:0} as unknown as Boss); renderBossUI(); fxState.hitStopFrames=10; fxState.screenShake=20; createExplosion(canvas.width/2,canvas.height/2,'#ffff00',40); spawnDebris(canvas.width/2,canvas.height/2,'#ffff00',8); },2000); }
